#ifndef _PPMD_MESH_HIERARCHY
#define _PPMD_MESH_HIERARCHY
#include <cmath>
#include <vector>
...
class MeshHierarchy {
  ...
  public:
    ...
    MeshHierarchy(SYCLTarget &sycl_target, const int ndim,
        ...
        PPMDASSERT(dims.size() >= ndim, "vector of dims too small");
        for (int dimx = 0; dimx < ndim; dimx++) {
            PPMDASSERT(dims[dimx] > 0, "Dim size is <= 0 in a direction.");
        }
        PPMDASSERT(cell_width_coarse > 0.0, "Extent <= 0.0 passed");
        PPMDASSERT(subdivision_order >= 0,
                   "Negative subdivision order passed.");
    };
};
